...
func LoadConfig(packageFile string) (*PkgConfig, error) {
	filename, _ := core.DownloadFile(packageFile)
...
func send(host string, p *PkgConfig) {
	...
	if !CmdFileExist(host, remoteFilePath) {
		...
		core.SendPackage(p.URL, []string{host}, p.Workspace, nil, nil)
	}
...
func (r *RunOnMaster) Send(config core.Config, p *PkgConfig) {
	...
	if p.ValuesContent != nil {
		core.SSHConfig.CopyConfigFile(config.Masters[0], p.Workspace+"/vaules.yml", p.ValuesContent)
	}
}

func (r *RunOnMaster) Run(config core.Config, p *PkgConfig) { // kubectl apply -f
	for _, cmd := range r.Cmd {
...
func CmdWorkSpace(node, cmd, workdir string) {
	...
	_ = core.SSHConfig.CmdAsync(node, command)
}

func CmdFileExist(node, path string) bool {
	return core.SSHConfig.IsFileExist(node, path)
}
